...
export const getPosts = async (req, res) =>{
    ...
    try{
        const LIMIT = 8; //broj postova po stranici
        const startIndex = (Number(page) -1) * LIMIT; //starting index of every page

        const total = await PostMessage.countDocuments({});
        const posts = await PostMessage.find().sort({ _id: -1}).limit(LIMIT).skip(startIndex); // najnoviji prvo
...
export const getPostsBySearch = async(req, res) => {
    ...
    try{
        ...
        const posts = await PostMessage.find({ $or: [{title},{name:title},{tags: {$in : tags.split(',')}}] });
